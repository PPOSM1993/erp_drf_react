...
from apps.accounts.models import UserGroups, GroupsPermissions
...
def check_permission(user, method, permission_to):
    ...
    for group in groups:
        permissions = GroupsPermissions.objects.values('permission_id').filter(group_id=group['group_id']).all()
